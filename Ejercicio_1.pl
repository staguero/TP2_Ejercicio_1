...
estado(estado_equipo_sin_efectos_oxidacion,ok).

%RAMA_CENTRAL_IZQUIERDA
estado(funcionamiento_correcto_piloto,desconocido).
estado(existe_prevencion_fuga_adecuada_asiento_orificio,desconocidok).
...
%RAMA_CENTRAL_DERECHA
estado(fuga_prevenible_entre_asiento_orificio,desconocido).
